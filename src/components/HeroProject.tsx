...
export default function HeroProject() {
  return (
    <section id="heroproject" className="min-h-screen bg-[#0a0a0a] text-white relative overflow-hidden">
      ...
      <div className="relative z-10 px-6 sm:px-12 lg:px-24 py-20 sm:py-32 max-w-7xl mx-auto">
        ...
        {/* Subheadline propio */}
        <p className="text-xl sm:text-2xl md:text-3xl text-white/60 max-w-3xl mb-16 leading-relaxed">
          "La secretaria marca el ingreso, la doctora recibe la notificación. 
          Nada de interrupciones, nada de papel."
        </p>

        {/* Stats grid */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 mb-16 border-t border-white/10 pt-12">
          <div>
            <p className="font-display text-4xl sm:text-5xl md:text-6xl font-semibold text-accent mb-2">
              50+
            </p>
            <p className="text-white/50 text-sm">Turnos semanales gestionados</p>
          </div>
          <div>
            <p className="font-display text-4xl sm:text-5xl md:text-6xl font-semibold mb-2">
              0
            </p>
            <p className="text-white/50 text-sm">Doble reserva desde la implementación</p>
          </div>
          <div>
            <p className="font-display text-4xl sm:text-5xl md:text-6xl font-semibold mb-2">
              3s
            </p>
            <p className="text-white/50 text-sm">Notificación de paciente listo</p>
          </div>
          <div>
            <p className="font-display text-4xl sm:text-5xl md:text-6xl font-semibold text-accent mb-2">
              PWA
            </p>
            <p className="text-white/50 text-sm">Instalada en móviles, sin app stores</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-16">
          <div className="bg-white/5 border border-white/10 p-6">
            ...
            <p className="text-white/50 text-sm">Telegram + Push nativas. La doctora sabe sin mirar el teléfono.</p>
          </div>
          <div className="bg-white/5 border border-white/10 p-6">
            <Calendar className="w-8 h-8 text-accent mb-4" />
            <h3 className="text-lg font-medium mb-2">Agenda sincronizada</h3>
            <p className="text-white/50 text-sm">Secretaría y consultorio siempre actualizados. Sin Excel.</p>
          </div>
          <div className="bg-white/5 border border-white/10 p-6">
            <Users className="w-8 h-8 text-accent mb-4" />
            <h3 className="text-lg font-medium mb-2">Fichas centralizadas</h3>
            <p className="text-white/50 text-sm">Historial, odontograma, evolución. Todo en un lugar.</p>
          </div>
        </div>

        {/* Tech stack */}
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-white/40 mb-12">
          <span>Next.js</span>
